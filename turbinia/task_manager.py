...
class CeleryTaskManager(BaseTaskManager):
  ...
  def enqueue_task(self, task, evidence_):
    ...
    task.stub = task_runner.apply_async((task.serialize(), evidence_.__dict__),
                                        expiration=86400, lease_duration_secs=86400)  # 24 hours
  ...
  def process_tasks(self):
    ...
    for task in self.tasks:
      ...
      if not psq_task:
        ...
        task.result = workers.TurbiniaTaskResult.deserialize(task.stub.result())
        completed_tasks.append(task)
